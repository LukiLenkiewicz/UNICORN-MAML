...
PRE_COMMON_PATH="pretrained/${DATASET}/Res12-pre.pth" # /con-pre.pth
BKB_CLASS=Res12 # /Conv4
N_SHOT=1
    ...
    for HN_WIDTH in 128 256 512; do
      for OPT in "sgd" "adam"; do
        for FREEZE_OPTION in "--hm_freeze_backbone" ""; do
          ...
          yes | $BASE_CMD  --model_class HyperMAML --para_init $PRE_PATH --hm_hn_len $HN_LEN --hm_hn_width ${HN_WIDTH} --optimizer_class ${OPT} $FREEZE_OPTION --suffix $SUFFIX &
        done
